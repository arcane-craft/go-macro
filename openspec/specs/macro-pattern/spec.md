...
- `EmbedMarker[$capture]`（如 `Derive[$iface]`）：MUST 存在**恰好一个**匿名 embed（`Names == nil`）其 invoked name 与 literal 一致；其它匿名 embed（非 marker，如 `io.Reader`）**MAY** 与 marker 共存，match 时 **MUST** 忽略（不参与 `$field ...` 绑定）。
- `$field ...`：MUST 绑定 struct 内**全部**具名字段，**不含**已匹配 marker embed，**不含**被忽略的其它匿名 embed；MAY 为 0 项。
- pattern 内 `{ Derive[$iface], $field ... }` 与 `{ $field ..., Derive[$iface] }` **MUST** 语义等价。
- 首版 **MUST NOT** 支持同一 struct 内多个 **marker** embed（invoked name 均匹配 pattern literal）；违反时 MUST 报错。
...
#### Scenario: 非 marker 匿名 embed 与 marker 共存

- **GIVEN** 源码 `type Item struct { io.Reader; provider.Derive[Stringer]; Name string }`，pattern `type $item struct { Derive[$iface] $field ... }`
- **WHEN** match 执行
- **THEN** MUST 成功；`io.Reader` 匿名 embed MUST 被忽略；`iface` 与 `Elems("field")`（长度 1，含 `Name string`）绑定 MUST 正确

#### Scenario: 多个 marker embed 报错

- **GIVEN** 源码 `type Item struct { Derive[A]; Derive[B] }`，pattern `type $item struct { Derive[$iface] }`
- **WHEN** match 执行
- **THEN** MUST 失败，且错误 MUST 表明存在多个 marker embed
...
| **CallPattern** | 单步，由 enclosing 容器字段决定；`AssignRhs` / `ExprSlot` 为 **`OneToOne`**（anchor 为槽位直接子节点或嵌于槽位表达式内均同 Plan）；`ReturnResults` 为 **`ReplaceAll`**（anchor 为 result 直接子节点）或 **`OneToOne`+`Index`**（anchor 嵌于 result 表达式内） |
| **DeclPattern** | `GenDeclSpecs` OneToOne + `InsertAfterInFileDecls` |

CallPattern 在 `ExprStmt` 父上下文中 MUST 使用 `ExprSlot` OneToOne（**不得**自动提升为 stmt 级）；stmt 级 1→N MUST 使用 StmtPattern `Try($inner);`。Apply 阶段子树 vs 整槽贴回规则见 macro-splice-apply「Apply 执行 SplicePlan」。
...
#### Scenario: CallPattern — assign RHS 嵌套表达式

- **GIVEN** 源码 `x := foo + Try(f())`，pattern `Try($inner)`（CallPattern）
- **WHEN** match 成功
- **THEN** `Plan` MUST 为 `AssignRhs` OneToOne；`MatchedSpan` MUST 为 anchor call

#### Scenario: CallPattern — return 嵌套 result

- **GIVEN** 源码 `return a, wrap(Try(f()))`，pattern `Try($inner)`（CallPattern）
- **WHEN** match 成功
- **THEN** `Plan` MUST 为 `ReturnResults` OneToOne（`Index=1`）；`MatchedSpan` MUST 为 anchor call

#### Scenario: CallPattern — return 直接 result

- **GIVEN** 源码 `return Try(f())`，pattern `Try($inner)`（CallPattern）
- **WHEN** match 成功
- **THEN** `Plan` MUST 为 `ReturnResults` ReplaceAll；`MatchedSpan` MUST 为 anchor call
