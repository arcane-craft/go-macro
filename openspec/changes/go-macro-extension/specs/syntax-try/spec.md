...
`try` 包 MUST 作为**官方宏库**发布：框架与 CLI 不默认启用；使用方 MUST 在宏主文件中 import `github.com/arcane-craft/go-macro/try` 后，`go tool macro expand` 方可展开 `Try` 族调用。

#### Scenario: 未 import 时不展开

- **WHEN** 宏主文件使用 `Try(...)` 但未 import `try` 包
...
错误路径上的 `return` SHOULD 使用外层具名标识符（若存在）。

#### Scenario: 具名 error

- **WHEN** 外层为 `func f() (data []byte, err error)` 且为 `Try` 赋值语境
- **THEN** 错误路径 SHOULD `return nil, err`
...
仓库 MUST 包含 `ReadFile` 示例（`Try` 用于 k=1），经 generate 后 `go test` 与 golden 一致。

#### Scenario: ReadFile 黄金测试

- **WHEN** 对示例执行 `go generate` 后运行 `go test`
