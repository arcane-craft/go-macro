...
**examples** module 的 `cmd/macroexpand` MUST 通过 blank import `_ "github.com/arcane-craft/go-macro/contrib/register"` 启用官方宏库 link。contrib MUST NOT 在根 module 的 `go.mod` 中被 require。

contrib MUST NOT 提供 `Main`/`Run` 作为用户 expand 入口（该职责在 `macro/expandtool`）。
