...
`go-macro` 仓库根 MUST 提供 `go.work`，且 `use` MUST 至少包含根 module 与 `./examples`，以便本仓联调。

对 `go-macro-contrib` 的本地并行开发，`examples/go.mod` SHOULD 使用 `replace github.com/arcane-craft/go-macro-contrib => ../go-macro-contrib`（新仓 checkout 位于 `go-macro` 仓库根的同级目录 `../go-macro-contrib`）。

#### Scenario: workspace 联调本仓两 module

- **WHEN** 开发者在 `go-macro` 仓库根执行 `go test ./...`（默认启用 `go.work`）
