...
定义 `go-macro` 仓库根目录 `README.md` 的信息架构与必备内容，使其面向人类读者可扫读、可分层深入，并与 `docs/author-guide.md` 职责分离。不改变框架运行时行为。
...
README MUST 链至 `docs/author-guide.md` 作为详细说明入口。README MUST NOT 复制 author-guide 级别的长契约正文（如完整 provider 实现说明）。
