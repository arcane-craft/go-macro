...
func ExpandPackages(patterns []string, providers []Provider) error {
	...
	for _, pkg := range pkgs {
		...
		if err := expandOnePackage(pkg, providers, expandedFiles); err != nil {
			return err
		}
	}
	return nil
...
func expandOnePackage(pkg *packages.Package, providers []Provider, expandedFiles map[string]bool) error {
	...
	active := mergeProviders(imported, providers)
	...
	if len(active) == 0 {
		// no macros in this package
	}
	if err := engine.RegisterProviders(active, filesByPath); err != nil {
		return err
	}
