...
`
	fset, file, _, info, pkg := parseSpliceTryFile(t, src)
	...
	ctx, err := macro.NewContext(fset, info, pkg, mc.Call, mc.StubName, mc.SyntaxID, site, enclosingFuncInFile(file, mc.Call))
	if err != nil {
		t.Fatal(err)
	}
...
`, parser.ParseComments)
	...
	if err := reg.RegisterProvider("example.com/try", []*ast.File{pf}, "syntax-try", try.TryExpand); err != nil {
		t.Fatal(err)
	}
	return reg
}
