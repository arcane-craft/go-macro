...
func TestModuleRoot(t *testing.T) {
	root, err := expander.ModuleRoot([]string{"github.com/arcane-craft/go-macro/expander"})
	if err != nil {
		t.Fatal(err)
	}
	if root == "" {
...
func TestFindImportedPackageViaExpandPackages(t *testing.T) {
	// Exercises providerFiles/findImportedPackage through real expand.
	if err := expander.ExpandPackages([]string{"github.com/arcane-craft/go-macro/examples/readfile"}, nil); err != nil {
		t.Fatal(err)
	}
}
