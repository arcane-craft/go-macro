...
`
	...
	ctx, err := macro.NewCallContext(fset, f, info, pkg, call, "Macro", "syntax-x", macro.SiteReturn, fn)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewContextInvalidEnclosing(t *testing.T) {
	fset := token.NewFileSet()
	_, err := macro.NewCallContext(fset, nil, nil, nil, nil, "X", "s", macro.SiteExpr, &ast.BlockStmt{})
	if err == nil {
